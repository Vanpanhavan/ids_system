...
def hash_the_password(password):
    ...
    hash_password_str = hash_password.hexdigest()
...
def log_activity(username, ip, status, port="Unknown", attempts=0):
    ...
    if status in ["FAILED", "SUSPICIOUS", "PORT_SCANNING", "BRUTE_FORCE", "SQL_INJECTION", "SUSPICIOUS_USER-AGENT"]:
...
def detect_unusual_user_agent(user_agent):
    """Detecting possible auto login with script or scraping"""
    suspicious_user_agent = ["python-requests", "nmap", "Hydra", "Python-urllib", "dirbuster", "Go-http-client", "sqlmap"]
    for agent in suspicious_user_agent:
...
def login():
    if request.method == "POST":
        ...
        password_hash = hash_the_password(password)
        ...
        if detect_sql_injection(username):
            log_activity(username, ip, "SQL_INJECTION", port, ip_attempts[ip])
            threat_detected = True
        
        if detect_unusual_user_agent(user_agent=user_agent):
            log_activity(username, ip, "SUSPICIOUS_USER-AGENT", port, ip_attempts[ip])
            threat_detected = True
